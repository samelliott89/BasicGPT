...
def load_datasets(
    ...
    val_split_percentage: float | None = None,
) -> IterableDataset:
    """
    ...
    if probabilities:
        ...
        print("  Using equal probability for all datasets")

    interleaved = InterleavedDataset(datasets=loaded_datasets, probabilities=probabilities)
...
if __name__ == "__main__":
    ...
    print(
        f"  ✓ Got sample with keys: {list(sample.keys()) if isinstance(sample, dict) else 'tensor tuple'}"
    )
    print()
    ...
    train_loader, _ = create_data_loaders(train_dataset=multi_dataset, batch_size=8, num_workers=0)
    ...
    assert input_ids.shape[1] == expected_seq_len, (
        f"Expected seq len {expected_seq_len}, got {input_ids.shape[1]}"
    )
    ...
    if max_input_token < tokenizer.vocab_size and max_target_token < tokenizer.vocab_size:
        ...
        print("  ⚠ WARNING: Some tokens exceed vocabulary size!")
    print()
    ...
    labeled_interleaved = InterleavedDataset(
        datasets=[labeled_synth, labeled_fineweb], probabilities=target_probs
    )
    ...
    for _sample, label in labeled_interleaved:
    ...
    for label, count in source_counts.items():
        ...
        print(
            f"  {label}: {count} samples ({actual_prob:.1%}) - expected {expected_prob:.0%} {status}"
        )
