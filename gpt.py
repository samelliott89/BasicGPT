            ...
            for _ in range(max_new_tokens):
                ...
                if repetition_penalty != 1.0:
                    ...
                    if generated.shape[1] > 0:
                        ...
                        next_token_logits[0, last_token] /= (
    ...
    for batch_idx, (input_ids, target_ids) in enumerate(train_loader):
        ...
        if (batch_idx + 1) % gradient_accumulation_steps == 0:
            ...
            if optimizer_steps % log_every_n_steps == 0:
                ...
                if device.type == "cuda":
                    ...
                    try:
                        ...
                        print(
                            f"  Step {optimizer_steps} (Batch {batch_idx + 1}/{loader_length}), Loss: {loss.item() * gradient_accumulation_steps:.4f}, "
                            f"GPU Memory: {memory_allocated:.2f}GB / {memory_reserved:.2f}GB"
                        )
                    except TypeError:
                        print(
                            f"  Step {optimizer_steps} (Batch {batch_idx + 1}), Loss: {loss.item() * gradient_accumulation_steps:.4f}, "
                            f"GPU Memory: {memory_allocated:.2f}GB / {memory_reserved:.2f}GB"
                        )
                else:
                    try:
            ...
            if (
                ...
                and optimizer_steps % val_check_interval == 0
            ):
        ...
        if (
            ...
            and training_config
            and current_total_batches % training_config.checkpoint_interval == 0
        ):
            timestamp = datetime.now().strftime("%m-%d-%Y-%H-%M")
            # Create folder for this checkpoint
            checkpoint_folder_name = (
                f"data-{data_config.max_samples}-batch-{current_total_batches}-{timestamp}"
            )
