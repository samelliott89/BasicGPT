...
# Optional W&B import
try:
    import wandb  # noqa: F401 - imported for availability check
...
def find_latest_checkpoint(save_dir: Path):
    ...
    # Look for checkpoint folders (data-*-batch-*-*)
    checkpoint_folders = list(save_dir.glob("data-*-batch-*"))

    if not checkpoint_folders:
        return None

    # Sort by batch number (extract from folder name)
    def get_batch_num(folder):
        try:
            # Format: data-10m-batch-40000-11-27-2025-08-52
            parts = folder.name.split("-batch-")
            if len(parts) >= 2:
    ...
    checkpoint_folders.sort(key=get_batch_num, reverse=True)
    latest_folder = checkpoint_folders[0]
    checkpoint_file = latest_folder / "checkpoint.pt"
...
def load_checkpoint(checkpoint_path, model, optimizer, scheduler, accelerator):
    ...
    print(f"✓ Resumed from epoch {epoch}, batch {total_batches:,}, loss {train_loss:.4f}")
    if wandb_run_id:
        print(f"  W&B run ID: {wandb_run_id}")

    return epoch, total_batches, wandb_run_id
        ...
        for batch_idx, (input_ids, target_ids) in enumerate(val_loader):
            ...
            with accelerator.autocast():
                ...
                loss = F.cross_entropy(
                    logits.reshape(-1, logits.size(-1)), target_ids.reshape(-1), reduction="sum"
                )
...
def main():
    ...
    # Check for existing checkpoint to get wandb run ID
    save_dir = Path(training_config.save_dir)
    resume_wandb_id = None
    latest_checkpoint = find_latest_checkpoint(save_dir)

    # First check for environment variable override (for resuming old runs)
    env_wandb_id = os.environ.get("WANDB_RUN_ID", None)
    if env_wandb_id and accelerator.is_main_process:
        resume_wandb_id = env_wandb_id
        print(f"📂 Using W&B run ID from environment: {resume_wandb_id}")
    elif latest_checkpoint is not None and accelerator.is_main_process:
        # Peek at checkpoint for wandb run ID (full load happens later)
        try:
            checkpoint_peek = torch.load(latest_checkpoint, map_location="cpu", weights_only=False)
            resume_wandb_id = checkpoint_peek.get("wandb_run_id", None)
            if resume_wandb_id:
                print(f"📂 Found checkpoint with W&B run ID: {resume_wandb_id}")
            del checkpoint_peek  # Free memory
        except Exception as e:
            print(f"⚠️ Could not peek checkpoint for wandb ID: {e}")

    # Initialize W&B (only on main process)
    if WANDB_AVAILABLE and accelerator.is_main_process:
        if resume_wandb_id:
            # Resume existing run
            run_name = None  # Will be restored from the run
            wandb_init_kwargs = {
                "wandb": {
                    "id": resume_wandb_id,
                    "resume": "must",
            ...
            print(f"✓ Resuming W&B run: {resume_wandb_id}")
        else:
            # Create new run
            run_name = f"gpt-{gpt_config.n_layers}L-{gpt_config.d_model}D-{datetime.now().strftime('%m%d-%H%M')}"
            wandb_init_kwargs = {"wandb": {"name": run_name}}
            print(f"✓ Starting new W&B run: {run_name}")
    ...
    # Get current wandb run ID to save in checkpoints
    ...
    _process_seed = 42 + accelerator.process_index  # noqa: F841 - kept for future use
    ...
    if accelerator.is_main_process:
        try:
            ...
            print("✓ Created train loader (streaming mode - batch count unknown)")
        try:
            print(f"✓ Created val loader with {len(val_loader)} batches")
        except TypeError:
            print("✓ Created val loader (streaming mode - batch count unknown)")
        print()
    ...
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer, lr_lambda=lambda step: get_lr(step, training_config.lr_config)
    )
    ...
    if accelerator.is_main_process:
        if accelerator.device.type == "cuda":
            ...
            print(
                f"  GPU Memory after model load: {torch.cuda.memory_allocated() / (1024**3):.2f}GB"
            )
        print()
    ...
    if latest_checkpoint is not None:
        ...
        start_epoch, total_batches = load_checkpoint(
            latest_checkpoint, model, optimizer, scheduler, accelerator
        )
        if accelerator.is_main_process:
            print(f"   Resuming from epoch {start_epoch}, batch {total_batches:,}")
            print()
    else:
        if accelerator.is_main_process:
    ...
    for epoch in range(start_epoch, training_config.epochs):
        ...
        if accelerator.is_main_process:
            ...
            max_samples_str = (
                f"{data_config.max_samples // 1_000_000}m" if data_config.max_samples else "all"
            )
    ...
    if accelerator.is_main_process:
        ...
        if WANDB_AVAILABLE:
            print("View training logs at: https://wandb.ai")
...
def train_epoch_accelerate(
    ...
    for batch_idx, (input_ids, target_ids) in enumerate(train_loader):
        ...
        if accelerator.sync_gradients:
            ...
            if optimizer_steps % log_every_n_steps == 0:
                ...
                if accelerator.is_main_process:
                    if accelerator.device.type == "cuda":
                        print(
                            f"  Step {optimizer_steps} (Batch {batch_idx + 1}), "
                            f"Loss: {loss.item():.4f}, LR: {current_lr:.2e}, "
                            f"GPU: {memory_allocated:.2f}GB"
                        )
                    else:
                        print(
                            f"  Step {optimizer_steps} (Batch {batch_idx + 1}), Loss: {loss.item():.4f}"
                        )
        ...
        if (
            current_total_batches % checkpoint_interval == 0
            ...
            checkpoint_folder_name = (
                f"data-{max_samples_str}-batch-{current_total_batches}-{timestamp}"
            )
            checkpoint_folder = save_dir / checkpoint_folder_name
            checkpoint_folder.mkdir(parents=True, exist_ok=True)
